...
StaticMesh::StaticMesh(HINSTANCE hInstance):D3DApp(hInstance)
{
}
...
void StaticMesh::Draw(const GameTimer& gt)
{
	...
	mCommandList->DrawIndexedInstanced(mBoxGeo->DrawArgs["StaticMesh"].IndexCount, 1, 0, 0, 0);
...
void StaticMesh::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void StaticMesh::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void StaticMesh::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void StaticMesh::ReadBinaryFileToStaticMeshStruct(char* TextPathName)
{
	...
	if (inFile.is_open()) {
		...
		inFile.read((char*)myStruct.Vertices.data(), verticesLen * sizeof(XMFLOAT3));
...
void StaticMesh::BulidDescriptorHeaps()
{
...
void StaticMesh::BulidConstantBuffers()
{
...
void StaticMesh::BulidRootSignature()
{
...
void StaticMesh::BulidShadersAndInputLayout()
{
	HRESULT hr = S_OK;
...
void StaticMesh::BuildStaticMeshGeometry()
{
	...
	mBoxGeo->DrawArgs["StaticMesh"] = submesh;
}

void StaticMesh::BuildPSO()
{
